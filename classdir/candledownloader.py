...
class CandleDownloader:
    ...
    def download_candles(self) -> None:
        ...
        validator = TimeframeValidator(self.output_file, self.timeframe)
    ...
    def _download_new_data(self) -> None:
        ...
        # Original download logic moved here
...
class TimeframeValidator:
    ...
    def __init__(self, csv_file: str, timeframe: str):
        ...
        self.logger = LoggerManager.setup_logger(
            f"{__name__}.TimeframeValidator",
            True,
            f'timeframe_validator_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        )
    ...
    def _log_results(self, result: Dict[str, Any]) -> None:
        ...
        if result['gaps']:
            ...
            for gap in result['gaps']:
                ...
                self.logger.warning(
                    f"Gap from {start_time} to {end_time} "
                    f"(size: {gap_td})"
                )
